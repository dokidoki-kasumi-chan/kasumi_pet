...
> v2.6: 过长消息直接拒绝（>2000字），60s 安全看门狗防止 THINKING 永久卡死。
v2.6.1: 深夜睡眠模式 — 00:00-08:00 zzz 动画，所有按钮隐藏。
v2.7: 开源发布。安装文档增加「已损坏」修复指引。设置面板增加模型下拉选择。
v2.8: 睡眠系统全面加固 — 启动直入 SLEEP、回答锁不挡睡眠、空白区域点击不打破睡眠。
